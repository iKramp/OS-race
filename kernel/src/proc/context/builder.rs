...
pub fn add_stack(context: &mut MemoryContext, stack_size_pages: u8) {
    ...
    'top_loop: for top_page in (0..stack_search_page).rev() {
        ...
        mem_tree.allocate_set_virtual(None, VirtAddr(top_page - stack_reserve_pages + 1));
        let entry = mem_tree.get_page_table_entry_mut(VirtAddr(top_page - stack_reserve_pages + 1)).unwrap();
        entry.set_writeable(false);
        entry.set_no_execute(true);
        entry.set_user_accessible(false);

        let stack = Stack {
            stack_base: VirtAddr(top_page  << 12),
