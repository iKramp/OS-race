...
impl NameString {
    pub fn new(data: &[u8]) -> (Self, u8) {
        if data[0] == ROOT_CHAR {
            ...
            return (Self::Rootchar(name_path.0), name_path.1)
        } else {
            let mut count = 0;
            ...
            return (Self::PrefixPath((count as u8, name_path.0)), name_path.1)
        }
    }
}
...
impl NamePath {
    pub fn new(data: &[u8]) -> (Self, u8) {//returns skip
        match data[0] {
            NULL_NAME => return (Self::NullName, 1),
            DUAL_NAME_PREFIX => return (Self::DualNamePath(DualNamePath::new(data)), 9),
            MULTI_NAME_PREFIX => {
                let ret_data = MultiNamePath::new(data);
                return (Self::MultiNamePath(ret_data.1), ret_data.0)
            }
            _ => {
                let typed_slie = unsafe { core::slice::from_raw_parts(&data[2] as *const _ as *const NameSeg, 1 as usize) };
                return (Self::NameSeg(typed_slie[0]), 4)
            }
        }
    }
}
...
impl DualNamePath {
    ...
    pub fn new(data: &[u8]) -> Self {
        ...
        return DualNamePath{ segments: [typed_slie[0], typed_slie[1]]}; 
    }
}
