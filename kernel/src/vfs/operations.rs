...
use super::{
    DeviceDetails, InodeIdentifier, InodeType, ROOT_INODE_INDEX, ResolvedPath, ResolvedPathBorrowed, VFS,
    file::{FileHandle, FileFlags},
...
async fn mount_filesystem(mountpoint: ResolvedPath, fs: &Arc<dyn FileSystem + Send>) -> Result<(), String> {
    ...
    if root {
        ...
        let parent_inode = fs_tree::get_inode_index((&mountpoint).into(), None)
            .await
...
pub async fn open_file(
    ...
    let inode_num = fs_tree::get_inode_index(path, from).await.ok_or("Path not found")?;
...
pub async fn read_file(file_handle: &mut FileHandle, buffer: &[PhysAddr], offset: u64, size: u64) -> Result<(), String> {
    ...
    let offset = file_handle.position + offset;
